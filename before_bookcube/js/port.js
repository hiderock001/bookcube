		...
		$(document).ready(function(){
			...
			//포트폴리오 ajax
			$.post("port.json", function(json){
				var list = json.list.all;	
				var _list = '';
		...
		var port = function(){
			...
			_gnb.on("click",function(){
				...
				if(_width > 767){
					...
					$('html, body:not(:animated)').animate({scrollTop: targetOffset}, 500);
				}
			});	

			$(".m-btn a").on("click",function(){
				if($(this).hasClass("on")){
		...
		$(function(){
			...
			var cir3 = $(".circle3");
			if(_width < 1100 ){
				$(".intro").css("background","transparent").remove();
				$("#wrap").show();
			}else{
